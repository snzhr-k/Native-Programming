...
void cheapest(const std::vector<Flight>& flights, std::map<std::string, unsigned>& map) {
  //collect the cheapest flight between the paor of cities
  //map<fromto, price>
  //LondonBudapest: 5000

  for (const auto& flight : flights) {
    std::string pair = flight.getFrom() + flight.getTo();
    auto it = map.find(pair);

    //key does not exist
    if (it == map.end()) {
      map[pair] = flight.getPrice();
    }

    //key does exist:
    else if (it->second > flight.getPrice()) {
      it->second = flight.getPrice();
    }
  }
}

//T3
unsigned count(const std::vector<Flight>& flights, const std::string& to_city) {
  return test::count_if(flights.begin(), flights.end(),
    [to_city](const Flight& fl) {return to_city == fl.getTo();});
}

//T4
std::vector<Flight> filter(const std::vector<Flight>& flights,
  const std::string& param1, const std::string& param2) {


  std::vector<Flight> result;

  // //has a minor problem with the reordering of the original order after filtering
  // for (const auto& flight : flights) {
  //   //outbound flights
  //   if (flight.getFrom() == param1 && flight.getTo() == param2) {
  //     result.insert(result.begin(), flight);
  //   }
  //   //return flights
  //   else if (flight.getFrom() == param2 && flight.getTo() == param1) {
  //     result.push_back(flight);
  //   }
  // }

  //keeps the original order after filtering
  for (const auto& flight : flights) {
    if (flight.getFrom() == param1 && flight.getTo() == param2) {
      result.push_back(flight);
    }
  ...
  for (const auto& flight : flights) {
    if (flight.getFrom() == param2 && flight.getTo() == param1) {
      result.push_back(flight);
    }
  }
...
//T7
std::vector<Flight> sales(const std::vector<Flight>& flights, const unsigned discount) {
  if (discount > 100) throw std::exception();
  std::vector<Flight> result;

  test::transform(flights.begin(), flights.end(), std::back_inserter(result),
    [discount](const Flight& flight) {
      unsigned new_price = flight.getPrice() * (100 - discount) / 100;
      return Flight(flight.getFrom(), flight.getTo(), new_price);
    });
